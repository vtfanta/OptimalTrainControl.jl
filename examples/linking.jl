...
trackX = [0.0,5e3,5.6e3,6e3,6.5e3,6.8e3,10e3]
trackY = getys(γs, trackX)
...
p0 = OldModelParams(mycontrol, (u, p, x) -> resistance(myresistance, u[2]), 
    (u, p, x) -> getgradientacceleration(steephilltrack, x), ρ, :MaxP)
sol = solve_regular([0.0,V,0.0], (3410, segs[3].finish), p0, segs[3])

plot(sol.t, sol[2,:])

xs = 2500:30:4000

vals = [try_link(x, segs[3]) for x in xs]
scatter(xs, clamp.(vals, minimum(filter(!isinf, vals)) - 5, maximum(filter(!isinf,vals)) + 5))

x_opt = find_zero(x -> try_link(x, segs[3]), [segs[2].start,segs[2].finish])
