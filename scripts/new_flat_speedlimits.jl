...
flat_odefun = OrdinaryDiffEq.ODEFunction(rhs_flat, mass_matrix=LinearAlgebra.Diagonal([1, 1, 0]))
 ...
 @kwdef mutable struct EETCSimParams{T<:Real}
    train::OptimalTrainControl.Train{T}
...
function get_init_E(mode::Mode, start_v::T, start_η::T, train::Train{T}, V::T, W::T) where {T<:Real}
    if mode == MaxP
...
function simulate(xspan::Tuple{T,T}, train::Train{T}, init_states::Vector, start_mode::Mode, V::T, cb) where {T<:Real}
...
#  P1 -> P2: MaxP to HoldP, 0 m -> 1307.69 m, found η(0) = 0.09206199978959455, within 1e-2 of the cruising speed
...
function root_start_to_hold(η₀, port1, port2, train::Train, V::T, startmode::Mode) where {T<:Real}
...
function root_start_to_finish(η₀, port1, port2, train::Train, V::T, startmode::Mode) where {T<:Real}
...
##
function link(port1, port2, train::Train, V::T; track=false) where {T<:Real}
    @assert port1.finish ≤ port2.start "Ports must be ordered: port1.finish ≤ port2.start"

    if isinf(port1.start)
