...
export TrainProblem, solve!

@with_kw struct TrainProblem
    umax = v -> 1 / max(5, v)
    umin = v -> -1 / max(5, v)
...
function solve!(prob::TrainProblem)
    ...
    spanV = (min(vᵢ, vf)+0.1, 140)

    Vopt = find_zero(time_constraint, spanV; atol = 5)

    params = NewModelParams(umax, umin, resistance, ρ, track, Vopt, vᵢ, vf)
    return findchain(getmildsegments(params), params)
end
